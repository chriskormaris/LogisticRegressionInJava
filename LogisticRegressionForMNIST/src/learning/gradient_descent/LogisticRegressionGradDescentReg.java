package learning.gradient_descent;
...
public class LogisticRegressionGradDescentReg {
	...
	private static final double lambda = 0.01; // the regularization parameter
	...
	public static double[] train(List<Instance> instances) {
		...
		for (int iter = 0; iter < maxiters; iter++) {
			...
			for (int i = 0; i < M; i++) { // for all train examples
				...
				J += -label * Math.log(predicted) - (1 - label) * Math.log(1 - predicted) + lambda * Utilities.sumOfSquares(weights) / (2 * M);
			}
